...
inline Ret read_scratchpad(Sensor& sensor, true_type) noexcept {
...
template<typename Ret, typename Sensor>
inline Ret read_scratchpad(Sensor& sensor, false_type) noexcept {
...
inline Ret read_scratchpad(Sensor& sensor) noexcept {
    ...
    return read_scratchpad<Ret>(
        sensor, integral_constant<bool, Sensor::with_decimal>{});
}
